...
bool prompts_and_inputs(string & filename, string & except, string & replaced, bool & isAFile)
{
        ...
        else
        {
                ...
                cout << "Enter the phrase you want to replace \"" << except << "\" with: ";
                getline(cin, replaced);

                ifstream foreignFile;
                foreignFile.open(replaced.c_str());
                if (foreignFile)
                {
                        if (!(checkExtension(replaced)))
                        {
                                throw 3;
                        }
                        foreignFile.close();
                ...
                else if (!foreignFile && checkExtension(replaced)) //the file ends in a .txt, but it does not exist
                {
                        throw 1;
                }
                else
...
int main ()
{
        ...
        while (!success && retry_count <= 3) //if the user enters a wrong file, it will try again three times.
        {
                try
                {
                        replacedIsAFile = prompts_and_inputs(fileInName, butEvery, isReplacedWith, replacedIsAFile);
                        success = true;
                }
                ...
                catch(int exception)
                {
                        if (exception == 1)
                        {
                                ...
                                if (retry_count < 3)
                                {
                                        cout << "WARNING " << exception << "." << retry_count << ": Non-existent file! Try again.\n";
                                }
        ...
        if (replacedIsAFile)
        {
                ...
                /*success = false;
                   ...
                   while (!success && retry_count <= 3) //if the user enters a wrong file, it will try agian three times.
                   {
                        ...
                        {
                                if (exception == "Incorrect file type!\n")
                                {
                                        cout << exception;
                                        return 1;
                                }
